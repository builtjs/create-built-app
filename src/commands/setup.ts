...
import { setupThemeData } from '../lib/setup/setup-theme/setup-theme';
...
async function setup(options: any) {
  ...
  let {type = Constants.TYPES.site} = options;
  if (type === Constants.TYPES.site) {
    ...
    if(theme){
      ...
      console.error('Error: This is not a theme (there is no theme.json file).')
    }
  ...
  //       combinedSectionPositionData
  //     );
  //     if(theme.plugins && Array.isArray(theme.plugins))
  //     setupImages(theme.plugins);
  //   })
  //   .catch(error => {
